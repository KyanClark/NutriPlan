...
class LocalFNRIService {
  ...
  static Future<List<FNRIIngredientNutrition>> loadFromCSV() async {
    ...
    _isLoading = true;
    try {
      AppLogger.info('📂 Attempting to load FNRI CSV from assets...');
      
      // Try loading the CSV file
      String csvContent;
      try {
        csvContent = await rootBundle.loadString('assets/data/fnri_detailed_nutritional_data.csv');
        AppLogger.info('✅ CSV file loaded successfully, size: ${csvContent.length} characters');
      } catch (e) {
        AppLogger.error('❌ Failed to load CSV file from assets', e);
        AppLogger.error('   Path attempted: assets/data/fnri_detailed_nutritional_data.csv');
      ...
      AppLogger.info('📊 Parsed ${lines.length} lines from CSV');
      
      if (lines.isEmpty) {
        AppLogger.warning('⚠️ CSV file is empty or has no valid lines');
        _isLoading = false;
        return [];
      }

      final headerRow = _parseCSVLine(lines[0]);
      AppLogger.info('📋 CSV header has ${headerRow.length} columns: ${headerRow.take(5).join(", ")}...');
      ...
      for (int i = 1; i < lines.length; i++) {
        try {
          ...
          if (i <= 5) {
            ...
            AppLogger.debug('⚠️ Skipped row $i: $e');
          }
          continue;
        }
      }
      ...
      AppLogger.info('✅ Successfully loaded ${ingredients.length} FNRI ingredients from CSV');
      if (skippedRows > 0) {
        AppLogger.warning('⚠️ Skipped $skippedRows invalid rows');
      }
      _isLoading = false;
      return ingredients;
    } catch (e, stackTrace) {
      AppLogger.error('❌ Critical error loading FNRI CSV', e);
