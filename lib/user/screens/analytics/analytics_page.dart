...
class _AnalyticsPageState extends State<AnalyticsPage> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  ...
  Widget _buildChartWithComparison(List<FlSpot> spots, List<String> sortedKeys, double dailyGoal) {
    ...
    return ClipRect(
      child: LineChart(_buildMonthlyChartData(spots, sortedKeys, dailyGoal, lastWeekSpots: lastWeekSpots, lastWeekKeys: lastWeekKeys)),
    );
  }
  ...
  LineChartData _buildMonthlyChartData(List<FlSpot> spots, List<String> sortedKeys, double dailyGoal, {List<FlSpot>? lastWeekSpots, List<String>? lastWeekKeys}) {
    // Calculate max Y value from actual data, with a ceiling
    final maxDataValue = spots.isEmpty ? dailyGoal : spots.map((s) => s.y).reduce((a, b) => a > b ? a : b);
    final calculatedMaxY = ((maxDataValue * 1.2).clamp(dailyGoal * 1.1, dailyGoal * 2.0).clamp(1000.0, 5000.0)).toDouble();
    
    return LineChartData(
      clipData: FlClipData.all(), // Clip all chart elements to prevent overflow
      lineTouchData: LineTouchData(
        enabled: true,
        touchTooltipData: LineTouchTooltipData(
          getTooltipItems: (touchedSpots) {
            return touchedSpots.map((barSpot) {
              final v = barSpot.y.clamp(0, calculatedMaxY); // Clamp tooltip value
      ...
      lineBarsData: [
        ...
        LineChartBarData(
          spots: spots.isEmpty 
              ...
              : spots.map((s) => FlSpot(s.x, s.y.clamp(0, calculatedMaxY))).toList(), // Clamp Y values to max
          isCurved: true,
          color: Colors.green[600],
          barWidth: 3,
          ...
          belowBarData: BarAreaData(
            ...
            cutOffY: calculatedMaxY.toDouble(), // Clip area fill at max Y
            applyCutOffY: true, // Enable clipping
          ),
