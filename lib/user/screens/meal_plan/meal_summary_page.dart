...
class MealSummaryPage extends StatefulWidget {
  ...
  final List<String>? initialMealTypes; // Pre-set meal types from auto-generate
...
class _MealSummaryPageState extends State<MealSummaryPage> {
  ...
  void initState() {
    super.initState();
    _mealsWithTime = widget.meals.asMap().entries.map((entry) {
      final index = entry.key;
      final recipe = entry.value;
      return RecipeWithTime(
        recipe: recipe,
        mealType: widget.initialMealTypes != null && index < widget.initialMealTypes!.length
            ? widget.initialMealTypes![index]
            : null,
        includeRice: false,
        riceServing: null,
      );
  ...
  bool _canBuildMealPlan() {
    final canBuild = _mealsWithTime.every((meal) {
      ...
      // Rice selection is optional - don't block meal saving if rice isn't selected
      // The validation only ensures that if includeRice is true, riceServing is set
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  if (meal.mealType != null && meal.time == null)
                                    const Text(''),
                                ],
                              ),
                              trailing: ElevatedButton(
                                onPressed: () {
                                  final mealTypeLocked = widget.initialMealTypes != null;
                                  if (mealTypeLocked) {
                                    // Meal type already set by auto-generate; go straight to rice/time selection
                                    _showRiceSelectionDialog(index);
                                  } else {
                                    _showMealTypeDialog(index);
                                  }
                                },
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: const Color(0xFF4CAF50),
