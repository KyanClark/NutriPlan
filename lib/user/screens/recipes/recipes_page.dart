...
import 'package:nutriplan/user/screens/meal_plan/meal_summary_page.dart';
import 'package:nutriplan/user/screens/meal_plan/meal_plan_confirmation_page.dart';
import 'package:nutriplan/user/screens/meal_plan/meal_planning_options_page.dart';
                              ...
                              if (userId != null) {
                                for (final m in mealsWithTime) {
                                  try {
                                    ...
                                    // Try to include rice data, but don't fail if columns don't exist
                                    ...
                                    if (e.toString().contains('include_rice') || e.toString().contains('rice_serving') || e.toString().contains('column') && e.toString().contains('does not exist')) {
                                      try {
                                        ...
                                        print('Failed to save meal to plans ${m.recipe.title}: $e2');
                                        if (mounted) {
                                          ScaffoldMessenger.of(context).showSnackBar(
                                            SnackBar(
